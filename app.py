...
# ---- تنظیمات ----
# کلید API را از متغیر محیطی بخوانید یا مستقیماً اینجا قرار دهید
# توصیه می شود از متغیر محیطی استفاده کنید: export OPENAI_API_KEY='sk-...'
# اگر از متغیر محیطی استفاده نمی کنید، خط زیر را فعال کرده و کلید خود را قرار دهید:
# os.environ['OPENAI_API_KEY'] = 'sk-PjFUL38fWAUG3emt6S8SPJo4mpzlzIeDcZ84tQq3DmriaN8i'

# بررسی کنید که کلید API تنظیم شده باشد
api_key = os.environ.get("OPENAI_API_KEY")
if not api_key:
    # اگر متغیر محیطی تنظیم نشده بود، از کلید پیش‌فرض استفاده کنید (توصیه نمی‌شود)
    api_key = 'sk-PjFUL38fWAUG3emt6S8SPJo4mpzlzIeDcZ84tQq3DmriaN8i'
    print("هشدار: از کلید API پیش‌فرض و ناامن استفاده می‌شود. لطفا متغیر محیطی OPENAI_API_KEY را تنظیم کنید.")
...
# ---- قالب HTML ----
# این یک قالب HTML ساده برای نمایش چت است
HTML_TEMPLATE = """
<!--
...
# ---- مدیریت تاریخچه مکالمه در سرور ----
# از یک دیکشنری برای ذخیره تاریخچه هر کاربر استفاده می کنیم
# در یک برنامه واقعی، بهتر است از پایگاه داده یا سیستم مدیریت نشست استفاده شود
conversation_history = {}

@app.route('/')
def home():
    # برای هر کاربر جدید، یک شناسه منحصر به فرد ایجاد می کنیم (مثلا با IP یا یک UUID)
    # برای سادگی، در اینجا از یک شناسه ثابت استفاده می کنیم.
    # در عمل، باید این بخش را برای مدیریت کاربران مختلف بهبود دهید.
    session_id = "default_user" # در یک برنامه واقعی، باید session_id را مدیریت کنید
...
def chat():
    session_id = "default_user" # مشابه بالا، شناسه سشن را مدیریت کنید
    user_message = request.json.get('message')

    if not user_message:
        return {"reply": "لطفا پیامی ارسال کنید."}, 400

    # دریافت تاریخچه مکالمه برای این کاربر
    ...
    try:
        ...
        # در صورت خطا، پاسخ مناسب را برگردانید
        return {"reply": f"متاسفانه خطایی در پردازش پیام شما رخ داد: {e}"}, 500

# ---- اجرای برنامه ----
if __name__ == '__main__':
    # برای اجرای برنامه، کافیست این اسکریپت را با پایتون اجرا کنید:
    # py app_flask.py
    # سپس به آدرس http://127.0.0.1:8000/ در مرورگر خود بروید.
